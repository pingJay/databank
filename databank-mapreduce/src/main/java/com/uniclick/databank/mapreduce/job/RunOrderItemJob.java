...
/**
 * 
 * @ClassName: RunOrderItemJob
 * @Description: TODO(广告定制报表)
 * @author ping.jie
 * @date 2015-4-14 下午12:19:40
 * @verision $Id
 *
 */
public class RunOrderItemJob implements RunCustomJob{
	private static final Logger logger = LoggerFactory.getLogger(RunOrderItemJob.class);
	private JobControl jobControl;
	
	public RunOrderItemJob(String queue) {
	...
	public boolean run(String beginDate,String endDate,String jobOptins,String resultnorm) {
		...
		String analyOutputDir = "hdfs://namenode/user/deepsight/tmp/"+beginDate + "---" + endDate +"/AnalyCustomizeOrderItem/output";
		String sumImpOutputDir = "hdfs://namenode/user/deepsight/tmp/"+beginDate + "---" + endDate +"/SumCustomizeOrderItemImp/output";
		String sumClkOutputDir = "hdfs://namenode/user/deepsight/tmp/"+beginDate + "---" + endDate +"/SumCustomizeOrderItemClk/output";
		String combineOutputDir = "hdfs://namenode/user/deepsight/tmp/"+beginDate + "---" + endDate +"/CombineCustomizeOrderItem/output";
		Path [] paths = null ;
		if(impStatus&&clkStatus){//同时分析展现和点击日志
			/**
			 * 分析展现日志
			 */
			...
			if(paths==null||paths.length==0){
				...
				if(!jobControl.AnalyOrderitemJob(paths, analyOutputDir, mrOptionModel.getOrder())){
					logger.error("RunOrderItemJob AnalyOrderitemJob imp proccess is faild");
					return false;
				}
				...
				if(!jobControl.SumInventoryJob(analyOutputDir, sumImpOutputDir, "imp")){
					logger.error("RunOrderItemJob SumInventoryJob imp proccess is faild");
					return false;
				}
			}
			/**
			...
			if(paths==null||paths.length==0){
				...
				if(!jobControl.AnalyOrderitemJob(paths, analyOutputDir, mrOptionModel.getOrder())){
					logger.error("RunOrderItemJob AnalyOrderitemJob clk proccess is faild");
					return false;
				}
				...
				if(!jobControl.SumInventoryJob(analyOutputDir, sumClkOutputDir, "clk")){
					logger.error("RunOrderItemJob SumInventoryJob clk proccess is faild");
					return false;
				}
			}
			/**
			...
			if(paths==null||paths.length==0){
				...
				if(!jobControl.CombineInventoryJob(paths, combineOutputDir,resultnorm)){
					logger.error("RunOrderItemJob CombineInventoryJob clk proccess is faild");
					return false;
				}
				//CommonUtil.ftpToLocal(beginDate.equals(endDate)?beginDate:beginDate+"_"+endDate, combineOutputDir, "orderitem",type);
			}
		...
		if(impStatus&&clkStatus){//同时分析展现和点击日志
			*//**
			...
			if(paths==null||paths.length==0){
				...
				JobControl.AnalyOrderitemJob(paths, analyOutputDir  , orderID);
				if(CommonUtil.judgeDir(sumImpOutputDir)){
					CommonUtil.delete(sumImpOutputDir);
				}
